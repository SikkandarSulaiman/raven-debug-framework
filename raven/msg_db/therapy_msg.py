...
class Message(Observable):
	...
	def __init__(self, data, priority=0, msgtype=0, payload=0, uniq_id=0, notify_now=False):
		self.therapymsg_t = namedtuple('therapymsg_t', 'start_byte priority uniq_id msg_id ack msg_type p0 p1 p2 p3 crc stop_byte')
	...
	def unpack_therapy_msg(self, msg_bytes):
		try: return self.therapymsg_t._make(unpack(self.tmsg_cstruct_fmt, msg_bytes))
		except: pass
	...
	def get_msg_name(self):
		try: msg_name = msg_db_val_to_name[str(self.f16.msg_id)]
		except: msg_name = f'{self.f16.msg_id:08X}'
		return msg_name
